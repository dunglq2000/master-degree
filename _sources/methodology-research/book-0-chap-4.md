...
Nghiên cứu lý thuyết bao gồm một số giai đoạn đặc trưng: – phân tích bản chất vật lý của các quá trình và hiện tượng; - xây dựng giả thuyết nghiên cứu; – xây dựng mô hình vật lý; – nghiên cứu toán học; – Phân tích và tổng hợp các nghiên cứu lý thuyết; - xây dựng kết luận.
...
Ở giai đoạn giới thiệu, nhà nghiên cứu sẽ kiểm kê chi tiết tất cả các kết quả mà mình có được, mọi thứ có thể quan tâm.
...
Поэтому важным этапом НИР является разработка методики исследования. Методика должна предусматривать теоретические и экспериментальные исследования.  
...
Các mô hình tự nhiên là các đối tượng được sửa đổi ở quy mô lớn; chúng cho phép nghiên cứu đầy đủ nhất các quá trình xảy ra trong điều kiện tự nhiên.
...
Một thử nghiệm tìm kiếm được thực hiện nếu việc phân loại các yếu tố ảnh hưởng đến hiện tượng đang nghiên cứu gặp khó khăn do thiếu dữ liệu sơ bộ (tiên nghiệm) đầy đủ. Dựa trên kết quả của thí nghiệm tìm kiếm, tầm quan trọng của các yếu tố được thiết lập và những yếu tố không đáng kể sẽ bị loại bỏ.
...
Trong tâm lý học, xã hội học và sư phạm, các thí nghiệm mở và đóng rất phổ biến.
...
Bản chất của thí nghiệm nhiều biến là tất cả các biến đều thay đổi cùng một lúc và mỗi tác động được đánh giá dựa trên kết quả của tất cả các thí nghiệm được thực hiện trong một chuỗi thí nghiệm nhất định.
...
4. При проведении расчетов в программе результат получается в виде некоторой цифровой информации, которую затем необходимо расшифровать. При вычислительном эксперименте точность информации определяется достоверностью модели, положенной в его основу, правильностью программ и алгоритмов для чего обычно проводятся предварительные «тестовые» испытания модели. 
...
Một phương pháp nghiên cứu thử nghiệm được phát triển đúng cách sẽ xác định giá trị của nó. Vì vậy, việc phát triển, lựa chọn và xác định phương pháp luận phải được thực hiện đặc biệt cẩn thận.
...
Ведение журнала, в котором фиксируются все характеристики исследуемого процесса и результаты наблюдений, является обязательным требованием проведения эксперимента. Также одновременно с проведением эксперимента исполнитель должен проводить предварительную обработку результатов и их анализ [3, 9]. 
...
Tất cả các yếu tố quyết định quá trình thay đổi đồng thời theo các quy tắc đặc biệt và kết quả thí nghiệm được trình bày dưới dạng mô hình toán học có một số tính chất tĩnh.
...
Biến đầu ra là những phản ứng hoặc phản ứng trước tác động của các tham số đầu vào. Chúng có thể là kinh tế (lợi nhuận, tiêu thụ năng lượng, v.v.), công nghệ (độ tin cậy, độ ổn định hồ quang, v.v.), v.v..
...
Các phép đo chiếm một vị trí cực kỳ quan trọng trong nghiên cứu thực nghiệm.

Đo lường là tìm giá trị của một đại lượng vật lý bằng thực nghiệm bằng các phương tiện kỹ thuật đặc biệt (GOST 1626370). Bản chất của phép đo là so sánh giá trị đo được với giá trị đã biết được lấy làm đơn vị, nghĩa là tiêu chuẩn. Để đo bất kỳ đại lượng vật lý Q nào có nghĩa là so sánh nó với một đại lượng q khác, được lấy làm đơn vị đo lường và biểu thị đại lượng đầu tiên dưới dạng phân số của đại lượng q đó.

Ở dạng toán học, điều này có thể được biểu diễn dưới dạng mối quan hệ

$$Q = kq,$$

trong đó $k$ là số nguyên dương hoặc số phân số bất kỳ cho biết Q lớn hơn hoặc nhỏ hơn $q$ bao nhiêu lần.

Đo lường liên quan đến lý thuyết và thực hành đo lường. Đây là khoa học về đo lường, phương pháp và phương tiện đảm bảo tính thống nhất của chúng và cách thức đạt được độ chính xác cần thiết. Các nguyên tắc cơ bản của đo lường bao gồm: – lý thuyết chung về phép đo; - đơn vị của đại lượng vật lý, nghĩa là các đại lượng mà theo định nghĩa được gán một giá trị bằng số bằng một. Ngoài ra, hệ thống của họ, tức là một tập hợp các đơn vị cơ bản và dẫn xuất được hình thành theo các nguyên tắc nhất định, ví dụ: Hệ thống đơn vị quốc tế - SI; - phương pháp và phương tiện đo. Phương pháp bao gồm tập hợp các kỹ thuật sử dụng nguyên lý, phương tiện kỹ thuật dùng trong đo lường và tiêu chuẩn hóa các đặc tính đo lường; - phương pháp xác định độ chính xác của phép đo; - cơ bản về đảm bảo tính đồng nhất của các phép đo. Kết quả đo phải được biểu thị bằng đơn vị pháp lý và sai số đo được biết với xác suất cho trước, điều này chỉ có thể xảy ra nếu các dụng cụ đo thống nhất. Chúng phải được hiệu chuẩn theo đơn vị pháp lý và các đặc tính đo lường của chúng phải tuân theo các tiêu chuẩn [3, 12].

Trong đo lường học, các tiêu chuẩn và dụng cụ đo tham chiếu là vô cùng quan trọng. Tiêu chuẩn được coi là các dụng cụ đo lường hoặc tổ hợp của chúng đảm bảo việc tái tạo và lưu trữ một đơn vị nhằm chuyển kích thước của nó sang các dụng cụ đo cấp thấp hơn. Các tiêu chuẩn được thực hiện theo một đặc điểm kỹ thuật đặc biệt. Ở Nga, cơ sở dữ liệu tiêu chuẩn chứa hơn 120 tiêu chuẩn nhà nước, bao gồm các đơn vị khối lượng, chiều dài, v.v. [3, 13].

Đó là những dụng cụ đo lường mẫu mực dùng để kiểm tra chúng bằng những dụng cụ đo lường kỹ thuật và làm việc thường xuyên được sử dụng trực tiếp trong nghiên cứu.

Việc chuyển kích thước đơn vị từ tiêu chuẩn hoặc dụng cụ đo mẫu sang dụng cụ làm việc được thực hiện bởi các cơ quan đo lường cấp bang và cấp sở tạo nên cơ quan đo lường của Nga (GOST 16263-70). Hoạt động của các cơ quan này ở nước ta đảm bảo tính thống nhất của các phép đo và tính thống nhất của các phương tiện đo.

Cơ quan đo lường của Nga được kết nối với toàn bộ hệ thống tiêu chuẩn hóa. Bản thân đo lường cung cấp các phương pháp xác định và giám sát các chỉ số, với sự trợ giúp của việc tiêu chuẩn hóa các phép đo được thực hiện và đảm bảo độ tin cậy cũng như khả năng so sánh của các chỉ số chất lượng có trong tiêu chuẩn. Điều này giải thích sự quan tâm lớn đến sự phát triển của dịch vụ đo lường.

Cơ quan đo lường ở nước ta là một mạng lưới rộng khắp các tổ chức khoa học, kiểm soát và thử nghiệm. Các cơ quan dịch vụ đo lường có khả năng thực hiện công việc quan trọng về các khía cạnh khoa học, lý thuyết và ứng dụng của các phép đo chính xác.

Hiện nay, công tác tiêu chuẩn hóa và đo lường trong nước do Ủy ban Tiêu chuẩn Nhà nước Nga (Gosstandart của Liên bang Nga) đứng đầu. Nhiệm vụ của nó bao gồm cải thiện hệ thống tiêu chuẩn hóa và đo lường, mở rộng phạm vi sử dụng để nâng cao hiệu quả trình độ kỹ thuật và chất lượng sản phẩm trong mọi lĩnh vực của nền kinh tế quốc dân, củng cố và phát triển dịch vụ đo lường nhà nước, tiêu chuẩn hóa phương pháp và dụng cụ đo lường, v.v.
...
Dịch vụ đo lường, tùy theo nhiệm vụ và chức năng mà nó thực hiện, được chia thành cấp bang và cấp cục.

Thực hiện công việc có hệ thống hàng ngày trong một bộ phận hoặc doanh nghiệp để đảm bảo tính thống nhất của các phép đo quốc gia là mục đích chính của dịch vụ đo lường, được tổ chức ở các bộ, ngành, tại các doanh nghiệp riêng lẻ, trong các viện nghiên cứu và trường đại học. Vì vậy, giữa cơ quan đo lường cấp sở và cơ quan đo lường cấp nhà nước có mối liên hệ không thể tách rời, cơ quan này là cơ quan chủ trì, quyết định, kiểm soát trong vấn đề này.

Các phép đo có thể là tĩnh, khi giá trị đo không thay đổi theo thời gian hoặc động. Các phép đo cũng có thể là trực tiếp hoặc gián tiếp. Trong các phép đo trực tiếp, giá trị mong muốn được xác định trực tiếp từ các thí nghiệm. Với những đại lượng gián tiếp - về mặt chức năng từ các đại lượng khác được xác định bằng các phép đo trực tiếp, ví dụ, đo mật độ của vật thể thông qua khối lượng và thể tích của nó. Ngoài ra còn có các phép đo tuyệt đối và tương đối. Tuyệt đối là các phép đo trực tiếp tính theo đơn vị của đại lượng được đo. Tương đối - các phép đo được biểu thị bằng tỷ lệ của giá trị đo được với cùng một giá trị, được lấy làm giá trị ban đầu.

Ngoài ra còn có ba loại phép đo: độ chính xác cao, độ chính xác cao và kỹ thuật.

Như đã đề cập trước đó, các phép đo là thành phần chính của bất kỳ thử nghiệm nào. Kết quả cuối cùng của thí nghiệm phụ thuộc vào độ kỹ lưỡng của các phép đo. Do đó, mọi nhà nghiên cứu phải có khả năng đo chính xác các đại lượng đang được nghiên cứu, biết mô hình của các quá trình được đo, đánh giá chính xác các sai số trong phép đo, xác định các điều kiện đo tốt nhất để sai số sẽ ở mức tối thiểu, tính toán các giá trị của số lượng và số lượng tối thiểu được yêu cầu của chúng, đồng thời tiến hành phân tích chung về kết quả đo [3].

Trong khí tượng học, có một số phương pháp đo chính.

Phương pháp đánh giá trực tiếp. Nó xác định giá trị của một đại lượng trực tiếp từ thiết bị đọc của thiết bị đo tác động trực tiếp (ví dụ: đo khối lượng trên thang quay số).

Phương pháp so sánh bằng thước đo. Khi sử dụng nó, giá trị đo được sẽ được so sánh với giá trị của thước đo có thể lặp lại (ví dụ: đo khối lượng trên cân đòn bẩy được cân bằng với trọng lượng).

Phương pháp số 0 được sử dụng để tạo ra tác động của một đại lượng tác động lên thiết bị về 0, ví dụ, đo điện trở bằng một cây cầu có sự cân bằng hoàn chỉnh của nó.

Phương pháp vi phân dựa trên thực tế là thiết bị đo bị ảnh hưởng bởi sự khác biệt giữa đại lượng đo được và đại lượng đã biết được tái tạo bằng thước đo, ví dụ, các phép đo được thực hiện khi kiểm tra thước đo chiều dài bằng cách so sánh với thước đo tiêu chuẩn trên bộ so sánh.

Phương pháp trùng hợp ngẫu nhiên. Sự khác biệt giữa giá trị đo được và giá trị của thước đo tái lập được đo bằng cách sử dụng các tín hiệu định kỳ hoặc sự trùng khớp của các dấu thang đo.

Phương pháp thay thế. Khi sử dụng nó, giá trị đo được thay thế bằng một giá trị đã biết, một thước đo có thể lặp lại, ví dụ, bằng cách cân cùng một đĩa cân bằng cách đặt khối lượng đo được và quả cân lần lượt.

Dụng cụ đo lường là một phần bắt buộc và không thể thiếu trong nghiên cứu thực nghiệm. Chúng là một tập hợp các phương tiện kỹ thuật có sai số chuẩn hóa nhằm cung cấp thông tin cần thiết cho người thực nghiệm.
...
Измерительная установка или стенд. Это особая система, состоящая из основных и вспомогательных средств измерений, предназначенных для измерения одной или нескольких величин. Установки могут вырабатывать сигналы, удобные для автоматической обработки результатов измерений. 

При проведении эксперимента иногда приходится создавать измерительные установки с фиксацией различных физических величин. 
...
Hiện nay, một số lượng lớn các dụng cụ đo và quan sát được sản xuất để đo các chỉ số cơ, lý, hóa, cũng như cấu trúc của các loại vật liệu, sản phẩm khác nhau, v.v.

Dụng cụ đo bao gồm dụng cụ đo, biện pháp, lắp đặt và hệ thống. Thước đo là phương tiện đo lường đơn giản nhất và nhằm mục đích tái tạo một đại lượng vật lý có kích thước nhất định, ví dụ: trọng lượng là thước đo khối lượng.

Ngoài ra còn có các dụng cụ đo cho phép bạn xác định trực tiếp chỉ số kiểm tra, ví dụ như máy ép để xác định độ bền của vật liệu. Các công cụ đo lường có thể đánh giá gián tiếp chỉ số đang được nghiên cứu, ví dụ, máy phát hiện lỗ hổng siêu âm, cho phép bạn đánh giá độ bền của vật liệu bằng tốc độ siêu âm.

Đo lắp đặt hoặc đứng. Đây là một hệ thống đặc biệt bao gồm các dụng cụ đo chính và phụ được thiết kế để đo một hoặc nhiều đại lượng. Việc cài đặt có thể tạo ra các tín hiệu thuận tiện cho việc xử lý tự động các kết quả đo.

Khi tiến hành một thí nghiệm, đôi khi cần phải tạo ra các thiết bị đo lường ghi lại các đại lượng vật lý khác nhau.

Thiết bị đo là một dụng cụ đo được thiết kế để tạo ra tín hiệu đo lường thông tin ở dạng mà người quan sát có thể tiếp cận được trực tiếp. Các đặc tính của dụng cụ đo là độ ổn định của phép đo, mức độ sai số và độ chính xác cũng như độ nhạy.

Tất cả các thiết bị được phân loại theo độ chính xác của phép đo, độ ổn định của số đọc, độ nhạy, giới hạn đo, v.v.

Độ chính xác của phép đo là mức độ gần đúng của phép đo với giá trị thực của giá trị đo được.

Sai số đo là sai số đại số giữa giá trị thực tế và giá trị đo được. Số lượng phép đo tối thiểu cung cấp giá trị trung bình ổn định của giá trị đo thỏa mãn mức độ chính xác nhất định.
...
где xи – показания прибора; xd – действительное значение измеряемой величины, полученное более точным методом. 
...
3) инструментальные, например, из-за износа инструмента, и т.д.;  
...
Các lỗi chính của thiết bị là tổng số lỗi được xác định trong điều kiện bình thường.

Để tăng độ tin cậy của phép đo và độ chính xác của chúng, cần phải giảm sai số. Sai số trong phép đo có thể phát sinh do một số nguyên nhân: ảnh hưởng của nhiều yếu tố bên ngoài trong quá trình thí nghiệm, thí nghiệm chưa kỹ lưỡng; sự không hoàn hảo của phương pháp và dụng cụ đo lường; đặc điểm chủ quan của người thực nghiệm, v.v.

Có những lỗi hệ thống và ngẫu nhiên. Có tính hệ thống là những lỗi không đổi sau nhiều lần thử nghiệm. Nếu biết các giá trị số của sai số, chúng phải được tính đến trong các thử nghiệm lặp lại. Lỗi hệ thống có thể được chia thành năm nhóm:

1) ảnh hưởng của môi trường bên ngoài: độ rung, từ trường và điện trường, độ ẩm, v.v.;
2) lắp đặt dụng cụ đo không chính xác;
3) dụng cụ, ví dụ, do mài mòn dụng cụ, v.v.;
4) phương pháp luận được chứng minh bằng việc lựa chọn phương pháp đo;
5) chủ quan.

Lỗi ngẫu nhiên. Chúng có thể xảy ra tình cờ trong quá trình đo lặp đi lặp lại. Những lỗi này không thể được tính đến và loại bỏ, nhưng với các phép đo lặp lại bằng phương pháp tĩnh, chúng có thể được xác định và loại bỏ.

Phạm vi đo của một thiết bị là một phần trong phạm vi đọc của thiết bị mà sai số của thiết bị được xác định. Với các lỗi đã biết của thiết bị, phạm vi đo và số đọc của thiết bị trùng khớp.

Khoảng của một dụng cụ là sự khác biệt giữa giá trị tối đa và tối thiểu của nó. Nếu đây là một giá trị không đổi, nghĩa là nếu trong hành trình đảo ngược có sự tăng hoặc giảm hành trình, thì sự khác biệt này được gọi là sự thay đổi của số đọc. Giá trị này là đặc tính đơn giản nhất của sai số dụng cụ.

Khả năng của thiết bị đọc phản ứng với những thay đổi trong giá trị đo được là một đặc tính khác của thiết bị và được gọi là độ nhạy. Ngưỡng độ nhạy của thiết bị là giá trị nhỏ nhất của giá trị đo được gây ra sự thay đổi số đọc của thiết bị có thể ghi lại được.

Một đặc điểm chính khác của thiết bị là độ chính xác của nó. Nó được đặc trưng bởi sai số tổng thể.

Tất cả các thiết bị được chia thành các lớp tùy thuộc vào lỗi cho phép. Lớp chính xác là một đặc tính tổng quát được xác định bởi giới hạn của các sai số chính và sai số bổ sung cho phép ảnh hưởng đến độ chính xác. Lớp chính xác thường được biểu thị bằng sai số cho phép tính bằng phần trăm (1–2, v.v.).
...
Все средства измерения, используемые в научных исследованиях, проходят обязательную периодическую поверку на точность. Поверка предусматривает уменьшение погрешностей прибора. Она позволяет установить соответствие данного прибора регламентированной степени точности, а также определить возможность его применения для данных измерений. 

При поверке средств измерения определяются погрешности и устанавливается, не выходят ли они за пределы допускаемых значений. 
...
Из вышесказанного можно сделать вывод о том, что очень важным моментом в организации научного эксперимента является выбор средств измерений. Средства измерения должны: – максимально соответствовать тематике, цели и задачам научно-исследовательской работы; – обеспечивать при проведении экспериментальных работ высокую производительность труда; – обеспечивать требуемое количество экспериментальных работ, то есть заданную степень точности при минимальном количестве измерений; – обеспечивать высокую воспроизводимость и надежность, по возможности исключать систематические ошибки, при этом желательно максимально использовать средства измерений с автоматической записью; – иметь высокую экономическую эффективность, то есть минимум затрат людских, денежных и материальных ресурсов; – обеспечивать эргономические требования эксперимента; – удовлетворять требованиям техники безопасности и пожарной безопасности. 
...
Khả năng tái tạo của thiết bị hoặc độ ổn định. Thuộc tính này của thiết bị đọc của thiết bị đảm bảo tính ổn định của các số đọc có cùng giá trị. Nó được xác định bởi sự thay đổi của bài đọc.

Tín hiệu ra của thiết bị đo được ghi lại bằng thiết bị đọc. Chúng có thể là kỹ thuật số, quy mô và ghi âm.

Một phần quan trọng của thiết bị là quy mô. Chiều dài vạch chia của thang đo là khoảng cách tính bằng milimét giữa hai vạch liền kề trên thang đo. Sự chênh lệch giữa các giá trị của đại lượng đo được tương ứng với điểm bắt đầu và kết thúc của thang đo được gọi là phạm vi số đọc của dụng cụ [3, 14, 15].

Tất cả các dụng cụ đo được sử dụng trong nghiên cứu khoa học đều phải trải qua quá trình kiểm tra độ chính xác định kỳ bắt buộc. Việc xác minh liên quan đến việc giảm lỗi của thiết bị. Nó cho phép bạn thiết lập sự tuân thủ của một thiết bị nhất định với mức độ chính xác được quy định, cũng như xác định khả năng sử dụng thiết bị đó cho dữ liệu đo lường.

Khi kiểm tra dụng cụ đo, sai số được xác định và xác định xem chúng có nằm ngoài giới hạn cho phép hay không.

Ở Nga, các viện đo lường và phòng thí nghiệm nhà nước để giám sát các tiêu chuẩn và thiết bị đo lường thực hiện kiểm soát nhà nước để đảm bảo tính thống nhất của các biện pháp. Tất cả các dụng cụ đo lường đều được kiểm tra 1–2 năm một lần.

Từ những điều trên chúng ta có thể kết luận rằng một điểm rất quan trọng trong việc tổ chức thí nghiệm khoa học là việc lựa chọn dụng cụ đo. Các công cụ đo lường phải: – tương ứng càng nhiều càng tốt với chủ đề, mục đích và mục tiêu của công việc nghiên cứu; – đảm bảo năng suất lao động cao trong quá trình làm thí nghiệm; – cung cấp khối lượng công việc thử nghiệm cần thiết, nghĩa là, mức độ chính xác nhất định với số lượng phép đo tối thiểu; – đảm bảo độ tái lập và độ tin cậy cao, loại bỏ các lỗi hệ thống bất cứ khi nào có thể, đồng thời mong muốn tận dụng tối đa các dụng cụ đo có khả năng ghi tự động; – có hiệu quả kinh tế cao, nghĩa là chi phí tối thiểu về nhân lực, tiền bạc và vật chất; – đảm bảo các yêu cầu về ecgônômi của thí nghiệm; – Đáp ứng các yêu cầu về an toàn và phòng chống cháy nổ.

Như vậy, yếu tố quan trọng nhất để thực hiện thành công nghiên cứu khoa học là hỗ trợ đo lường cho nghiên cứu khoa học và đặc biệt là đảm bảo tính thống nhất của phép đo và tính đồng nhất của các dụng cụ đo. Vì vậy, nếu không có sự phát triển thành công của đo lường thì không thể có tiến bộ trong phát triển khoa học và ngược lại, nếu không có sự phát triển thành công của khoa học thì không thể có tiến bộ trong đo lường.

## 4.6. Организация рабочего места экспериментатора 

Рабочее место экспериментатора – это часть рабочего пространства, на которое распространяется его непосредственное воздействие в процессе исследования. 

Рабочим пространством называется часть лабораторного или производственного помещения, оснащенная необходимыми экспериментальными средствами и обслуживаемая одним или группой исследователей. Рабочее пространство может быть: – стационарным, например лаборатория, научно-исследовательское учреждение, полигоны и т.п.; – мобильным, например ходовые лаборатории; – условно-стационарным, например передвижные лаборатории, временные полигоны. 

Лабораторией является специально оборудованное помещение, в котором производятся экспериментальные исследования. В соответствии с особенностями рабочего пространства выделяют три типа исследовательских лабораторий: стационарные, передвижные и ходовые. 

В стационарной лаборатории рабочее место комплектуется специальным рабочим столом. В зависимости от назначения лаборатории каждый лабораторный стол должен обеспечиваться электричеством, газом, водой, паром, сжатым воздухом и общим вакуумом. На столах размещаются штепсели для включения электроприборов, компьютеров, настольных ламп, нагревательных приборов (паяльники, плитки), размещенных на кусках толстого листового асбеста. Освещенности рабочего места следует уделять особое внимание. 

Оборудование передвижных лабораторий должно быть приближено к стационарным, но несколько уступает им из-за нехватки площадей. 

Например, вместо лабораторного стола передвижная лаборатория оснащается откидным столиком для ведения необходимых записей в процессе проведения эксперимента. 

Экспериментатор в лаборатории выполняет весьма ответственную работу. От неё часто зависит правильность решения теоретической или практической задачи в целом. Главными условиями эффективной экспериментальной работы являются: аккуратность, тщательность подготовки эксперимента, точность при выполнении предписаний методики, внимательность при проведении эксперимента. Исследователь, приступая к проведению эксперимента, должен еще раз обдумать и уточнить методику, подготовить всю необходимую документацию (акты, лабораторные тетради, журналы), которая предназначена для регистрации хода и результатов опытов. 

Все наблюдения, определения и анализы необходимо записывать в специальный журнал. Его форма должна соответствовать исследуемому процессу с максимальной фиксацией всех фактов и условий их появления. Исполнитель должен при получении в одном статистическом ряду результатов, резко отличающихся от соседних измерений, записать все данные без искажений и указать обстоятельства, которые сопутствуют указанному измерению. В дальнейшем это позволит установить причины отклонений и соответствующим образом классифицировать их. Если необходимо в процессе измерения произвести простейшие расчеты, то они должны быть внесены в журнал или в отдельную тетрадь с указанием даты проведения опыта, номера и серии опыта. 

Лабораторные журналы и тетради являются важнейшими первичными документами исследователя, поэтому должны содержаться в порядке и обеспечивать возможность логической проверки. Нужно стремиться не допускать в них исправлений, а при необходимости исправления должны делаться так, чтобы не происходило путаницы при расчетах. Любое исправление должно сопровождаться пояснением экспериментатора и краткой справкой о причинах исправлений. В лабораторных журналах и тетрадях не следует делать записей или пометок, не относящихся к делу. 

Исполнитель обязан систематически проводить поверку средств измерений. При проведении эксперимента исполнитель должен непрерывно следить за средствами измерений, правильностью их показаний, характеристикой окружающей среды, устойчивостью аппаратов и установок и не допускать посторонних лиц в рабочую зону.   

Nơi làm việc của người thực nghiệm là một phần không gian làm việc chịu ảnh hưởng trực tiếp của anh ta trong quá trình nghiên cứu.

Không gian làm việc là một phần của phòng thí nghiệm hoặc cơ sở sản xuất được trang bị các công cụ thí nghiệm cần thiết và được phục vụ bởi một hoặc một nhóm nhà nghiên cứu. Không gian làm việc có thể là: – cố định, ví dụ như phòng thí nghiệm, cơ sở nghiên cứu, bãi thử nghiệm, v.v.; – di động, ví dụ phòng thí nghiệm di động; – cố định có điều kiện, ví dụ, phòng thí nghiệm di động, bãi thử nghiệm tạm thời.

Phòng thí nghiệm là một căn phòng được trang bị đặc biệt để tiến hành nghiên cứu thực nghiệm. Phù hợp với đặc điểm của không gian làm việc, ba loại phòng thí nghiệm nghiên cứu được phân biệt: cố định, di động và có thể đi bộ.

Trong phòng thí nghiệm cố định, nơi làm việc được trang bị một bàn làm việc đặc biệt. Tùy thuộc vào mục đích của phòng thí nghiệm, mỗi bàn thí nghiệm phải được cung cấp điện, gas, nước, hơi nước, khí nén và chân không chung. Các phích cắm được đặt trên bàn để bật các thiết bị điện, máy tính, đèn bàn, thiết bị sưởi ấm (bàn là hàn, bếp nấu), đặt trên các miếng amiăng tấm dày. Cần chú ý đặc biệt đến ánh sáng nơi làm việc.

Thiết bị của phòng thí nghiệm di động phải gần với phòng thí nghiệm cố định nhưng có phần kém hơn do thiếu không gian.

Ví dụ, thay vì bàn thí nghiệm, phòng thí nghiệm di động được trang bị bàn gấp để lưu giữ những ghi chú cần thiết trong quá trình thí nghiệm.

Một người thí nghiệm trong phòng thí nghiệm thực hiện một công việc rất có trách nhiệm. Tính đúng đắn của việc giải quyết một vấn đề lý thuyết hoặc thực tiễn nói chung thường phụ thuộc vào nó. Các điều kiện chính để công việc thí nghiệm hiệu quả là: tính chính xác, kỹ lưỡng trong việc chuẩn bị thí nghiệm, tính chính xác trong việc tuân thủ các hướng dẫn của phương pháp và sự cẩn thận khi tiến hành thí nghiệm. Khi bắt đầu thử nghiệm, nhà nghiên cứu phải suy nghĩ lại và làm rõ phương pháp, chuẩn bị tất cả các tài liệu cần thiết (báo cáo, sổ ghi chép trong phòng thí nghiệm, tạp chí), nhằm ghi lại tiến độ và kết quả của thí nghiệm.

Tất cả các quan sát, xác định và phân tích phải được ghi lại trong nhật ký đặc biệt. Hình thức của nó phải tương ứng với quá trình nghiên cứu với sự ghi lại tối đa tất cả các sự kiện và điều kiện xảy ra của chúng. Người thực hiện phải, khi nhận được các kết quả trong một chuỗi thống kê có sự khác biệt lớn so với các phép đo lân cận, phải ghi lại tất cả dữ liệu mà không làm biến dạng và chỉ ra các trường hợp đi kèm với phép đo được chỉ định. Trong tương lai, điều này sẽ cho phép chúng tôi xác định nguyên nhân gây ra sai lệch và phân loại chúng cho phù hợp. Nếu cần thực hiện các phép tính đơn giản trong quá trình đo thì phải ghi vào nhật ký hoặc sổ riêng ghi rõ ngày thí nghiệm, số lượng và chuỗi thí nghiệm.

Nhật ký và sổ ghi chép trong phòng thí nghiệm là tài liệu chính quan trọng nhất của nhà nghiên cứu, do đó chúng phải được lưu giữ ngăn nắp và có khả năng xác minh hợp lý. Chúng ta phải cố gắng tránh sửa chữa chúng và nếu cần thiết, việc sửa chữa phải được thực hiện theo cách không gây nhầm lẫn trong tính toán. Bất kỳ sự điều chỉnh nào đều phải kèm theo lời giải thích của người thực nghiệm và tuyên bố ngắn gọn về lý do điều chỉnh. Không được ghi hoặc ghi chú những điều không liên quan vào nhật ký và sổ ghi chép trong phòng thí nghiệm.

Nhà thầu có nghĩa vụ kiểm tra một cách có hệ thống các dụng cụ đo lường. Khi tiến hành thí nghiệm, người thực hiện phải liên tục theo dõi các dụng cụ đo, tính chính xác của số đọc, đặc điểm của môi trường, độ ổn định của thiết bị và lắp đặt và không cho phép những người không có thẩm quyền vào khu vực làm việc.

---

Творческие особенности экспериментатора должны проявляться при предварительной обработке результатов и их анализе. Такой анализ позволяет контролировать исследуемый процесс, улучшать методику, корректировать эксперимент и повышать его эффективность. 

В процессе экспериментальных работ необходимо соблюдать инструкции по промсанитарии, требования техники безопасности, пожарной профилактики. Особое внимание следует уделять уменьшению шума при эксперименте, состоянию газовых кранов и электрооборудования. 

Газовые краны должны периодически проверяться специалистами на утечку газа. Все электроприборы должны быть заземлены.  

При выполнении производственных экспериментов необходимо очень строго соблюдать все перечисленные требования. Вследствие больших объемов работ и значительной их трудоемкости ошибки, которые были допущены в процессе эксперимента, могут значительно увеличить продолжительность исследований и соответственно уменьшить их точность. 

Все результаты измерений сначала сводят в таблицы по варьирующим характеристикам, потом тщательно изучают сомнительные цифры, которые резко отличаются от статистического ряда наблюдений. 

При анализе цифр необходимо установить точность, с которой нужно производить обработку опытных данных, то есть точность обработки не должна быть выше точности измерений. 

Важное место при проведении экспериментальных исследований занимает анализ результатов эксперимента. Это завершающая часть, на базе которой делается вывод о подтверждении гипотезы научного исследования. Анализ эксперимента является творческой частью исследования. Поскольку за цифрами иногда трудно представить физическую сущность процесса, требуется особо тщательное сопоставление причин, фактов, обусловливающих ход того или иного процесса и установление адекватности гипотезы и эксперимента. 

Результаты некоторых лабораторных и большинства производственных экспериментов оформляются протоколом, который подписывается экспериментатором и руководителем производства [3]. 

Các tính năng sáng tạo của người thử nghiệm phải được thể hiện trong quá trình xử lý sơ bộ kết quả và phân tích chúng. Phân tích như vậy cho phép bạn kiểm soát quá trình nghiên cứu, cải tiến phương pháp luận, điều chỉnh thử nghiệm và tăng hiệu quả của nó.

Trong quá trình thí nghiệm phải tuân thủ các hướng dẫn vệ sinh công nghiệp, yêu cầu an toàn, phòng chống cháy nổ. Cần đặc biệt chú ý đến việc giảm tiếng ồn trong quá trình thí nghiệm, tình trạng vòi gas và thiết bị điện.

Vòi gas cần được các chuyên gia kiểm tra định kỳ xem có rò rỉ gas không. Tất cả các thiết bị điện phải được nối đất.

Khi thực hiện thí nghiệm sản xuất phải tuân thủ nghiêm ngặt tất cả các yêu cầu trên. Do khối lượng công việc lớn và cường độ lao động đáng kể, các lỗi mắc phải trong quá trình thử nghiệm có thể làm tăng đáng kể thời gian nghiên cứu và do đó làm giảm độ chính xác của nó.

Tất cả các kết quả đo lường trước tiên được lập bảng theo các đặc điểm khác nhau, sau đó các số liệu đáng ngờ khác biệt rõ rệt với chuỗi quan sát thống kê sẽ được nghiên cứu cẩn thận.

Khi phân tích các con số, cần thiết lập độ chính xác mà dữ liệu thực nghiệm phải được xử lý, nghĩa là độ chính xác của quá trình xử lý không được cao hơn độ chính xác của phép đo.

Một vị trí quan trọng khi tiến hành nghiên cứu thực nghiệm là phân tích kết quả thực nghiệm. Đây là phần cuối cùng, trên cơ sở đó đưa ra kết luận về việc xác nhận giả thuyết nghiên cứu khoa học. Phân tích thí nghiệm là phần sáng tạo của nghiên cứu. Vì đôi khi rất khó để tưởng tượng bản chất vật lý của quá trình đằng sau những con số, nên cần phải có sự so sánh đặc biệt cẩn thận về các nguyên nhân và sự kiện xác định diễn biến của một quá trình cụ thể cũng như việc thiết lập tính thỏa đáng của giả thuyết và thí nghiệm.

Kết quả của một số thí nghiệm trong phòng thí nghiệm và hầu hết các thí nghiệm sản xuất được ghi lại trong một quy trình được ký bởi người thí nghiệm và người quản lý sản xuất [3].
...
Иногда в серии одинаковых измерений встречаются измерения с очень большими случайными ошибками, имеющими малую вероятность. Такие измерения относят к промахам экспериментатора и затем отбрасывают. Но при этом не нужно забывать, что, хотя существует очень малая вероятность того, что отброшенное число является не промахом, а естественным статистическим отклонением, как правило, пренебрежение им не приводит к существенному ухудшению оценки результатов измерений. Действительно, в процессе эксперимента иногда бывает трудно отделить систематические погрешности от случайных. 

Однако при многократном и тщательном выполнении эксперимента этого результата можно достичь. 

Главная задача исследователя провести измерения с наименьшими погрешностями, с использованием всех возможных методов устранения систематических и случайных ошибок. 

Систематические погрешности можно разделить на следующие группы: – инструментальные погрешности, которые появляются вследствие нарушений средств измерений из-за неточности градировочных шкал, износа и старения узлов и деталей средств измерений, возникающих по причине их неправильной установки; – погрешности, которые возникают в результате действия внешних факторов (высокая температура воздуха, атмосферное давление и влажность воздуха, магнитные и электрические поля, вибрация и колебания от движущегося транспорта и др.); – субъективные погрешности, которые могут возникнуть вследствие индивидуальных, психофизиологических, физиологических, антропологических свойств человека. 
...
Бывает, что ошибки эксперимента связаны с тем, что исследователь не представляет себе четко, что он собирается получить. В результате могут быть не учтены важнейшие факторы, а это существенно затруднит анализ экспериментальных данных.

Иногда в процессе анализа результатов эксперимента исследователь бессознательно подгоняет экспериментальные данные для подтверждения ранее выдвинутой гипотезы. Эта опасность особенно велика, если вывод делается на основании данных, на которых могут существенно сказываться ошибки измерения и влияние неучитываемых факторов. В таких условиях нетрудно подобрать достаточное количество фактов, подтверждающих принятую гипотезу, объяснить заметные отклонения промахами и тем самым уйти от истины. Для исключения таких ошибок известный физик Резерфорд проводил серии опытов, показатели которых учитывали студенты, не знавшие, в чем заключается опыт, а кривые по полученным точкам проводили другие люди, также не знавшие, что должно получиться. Применение такой методики обработки материалов эксперимента позволило Резерфорду и его ученикам не сделать ни одного ошибочного открытия, в то время как в других лабораториях таких «открытий» было немало. 

Khi tiến hành thí nghiệm, các phép đo của các chỉ số khác nhau không thể được thực hiện chính xác tuyệt đối vì bản thân dụng cụ đo có một sai số nhất định. Sai số đo lường có thể phát sinh do thử nghiệm không đủ kỹ lưỡng, phương pháp và dụng cụ đo không hoàn hảo, ảnh hưởng của các yếu tố không được tính đến khác nhau trong quá trình thử nghiệm và cuối cùng là đặc điểm chủ quan của bản thân nhà nghiên cứu.

Ở trên đã lưu ý rằng sai số đo lường được phân loại thành sai số hệ thống và sai số ngẫu nhiên. Các lỗi hệ thống trong các thử nghiệm lặp lại không đổi và nếu biết giá trị số của các lỗi này, chúng có thể được tính đến trong các phép đo lặp lại. Lỗi ngẫu nhiên có thể xảy ra do đo lặp đi lặp lại. Nhưng với sự lặp lại nhiều lần, các phương pháp thống kê có thể loại bỏ các phép đo ngẫu nhiên sai lệch nhất.

Một loại lỗi ngẫu nhiên có thể là lỗi tổng thể vượt quá đáng kể lỗi hệ thống hoặc lỗi ngẫu nhiên. Những lỗi và sai sót thô thiển như vậy thường do lỗi của người thí nghiệm gây ra. Chúng có thể dễ dàng được phát hiện và sau đó bị bỏ qua trong quá trình phân tích. Vì vậy, cần đặc biệt lưu ý rằng việc thu thập và xử lý số liệu thống kê đòi hỏi người nghiên cứu phải hết sức chú ý và có kỹ năng phù hợp.

Đôi khi trong một loạt các phép đo giống nhau có những phép đo có sai số ngẫu nhiên rất lớn và có xác suất thấp. Các phép đo như vậy được cho là do lỗi của người thực nghiệm và sau đó bị loại bỏ. Nhưng chúng ta không được quên rằng, mặc dù có xác suất rất nhỏ là số bị loại bỏ không phải là sai, nhưng độ lệch thống kê tự nhiên, theo quy luật, việc bỏ qua nó sẽ không dẫn đến sự suy giảm đáng kể trong việc đánh giá kết quả đo. Thật vậy, trong quá trình thử nghiệm, đôi khi rất khó để phân biệt các sai số hệ thống với các sai số ngẫu nhiên.

Tuy nhiên, với việc thực hiện thí nghiệm nhiều lần và cẩn thận, kết quả này có thể đạt được.

Nhiệm vụ chính của nhà nghiên cứu là thực hiện các phép đo với sai số nhỏ nhất, sử dụng tất cả các phương pháp có thể để loại bỏ các sai số hệ thống và ngẫu nhiên.

Các lỗi hệ thống có thể được chia thành các nhóm sau: – các lỗi của dụng cụ xuất hiện do sự vi phạm của các dụng cụ đo do cân hiệu chuẩn không chính xác, sự hao mòn và lão hóa của các bộ phận và bộ phận của dụng cụ đo phát sinh do lắp đặt không đúng; – các lỗi phát sinh do các yếu tố bên ngoài (nhiệt độ không khí cao, áp suất khí quyển và độ ẩm không khí, từ trường và điện trường, độ rung và dao động từ các phương tiện đang chuyển động, v.v.); - những sai sót chủ quan có thể phát sinh do các đặc điểm cá nhân, tâm sinh lý, sinh lý, nhân học của một người.

Trong số các sai số đo lường, sai số chủ quan chiếm một vị trí quan trọng. Nguồn gốc của chúng là lý do tâm lý hoặc tâm sinh lý. Ví dụ, do khiếm khuyết về thị giác, người thí nghiệm có thể không đọc chính xác các chỉ số của dụng cụ. Để loại bỏ các lỗi như vậy, việc cung cấp ánh sáng cần thiết và chọn mức chia độ thích hợp của thang đo dụng cụ là đủ.

Nguyên nhân tâm lý của sai sót còn bao gồm quán tính của suy nghĩ và nhiều rào cản tâm lý khác nhau. Thông thường, nhà nghiên cứu cố gắng hiểu các kết quả thử nghiệm mới và bất ngờ trong khuôn khổ các ý tưởng cũ, nhưng chúng không phù hợp với khuôn khổ này và bị anh ta coi là sai lầm. Ở đây quán tính trong suy nghĩ của người thử nghiệm được thể hiện, tức là niềm tin của anh ta vào sự hoàn hảo và phổ biến của những ý tưởng cũ, hoặc có thể chỉ là nỗi sợ hãi trước cái mới.

Điều xảy ra là các sai sót trong thí nghiệm là do nhà nghiên cứu không hiểu rõ ràng những gì mình sẽ thu được. Kết quả là, các yếu tố quan trọng nhất có thể không được tính đến và điều này sẽ làm phức tạp đáng kể việc phân tích dữ liệu thực nghiệm.

Đôi khi, trong quá trình phân tích kết quả của một thí nghiệm, nhà nghiên cứu đã vô thức điều chỉnh dữ liệu thực nghiệm để xác nhận một giả thuyết được đưa ra trước đó. Mối nguy hiểm này đặc biệt lớn nếu kết luận được đưa ra trên cơ sở dữ liệu có thể bị ảnh hưởng đáng kể bởi sai số đo lường và ảnh hưởng của các yếu tố không được tính toán. Trong những điều kiện như vậy, không khó để chọn ra đủ số lượng sự kiện xác nhận giả thuyết được chấp nhận, giải thích những sai lệch đáng chú ý là sai lầm và từ đó trốn tránh sự thật. Để loại bỏ những sai sót như vậy, nhà vật lý nổi tiếng Rutherford đã tiến hành một loạt thí nghiệm, các chỉ số của chúng được những sinh viên không biết thí nghiệm bao gồm những gì tính đến và các đường cong được vẽ từ những điểm thu được bởi những người khác cũng không biết. biết điều gì sẽ xảy ra. Việc sử dụng kỹ thuật xử lý vật liệu thí nghiệm như vậy đã cho phép Rutherford và các sinh viên của ông không mắc phải một khám phá sai lầm nào, trong khi ở các phòng thí nghiệm khác đã có rất nhiều “khám phá” như vậy.

---

Всё вышеизложенное доказывает, что любой результат эксперимента должен многократно проверяться и восприниматься критически. Перепроверку результатов эксперимента целесообразнее осуществлять в другое время дня или по истечении нескольких дней. 

После завершения всех серий эксперимента исследователь может принять решение: признать ли основную часть работы законченной; есть ли необходимость провести дополнительный сбор информации и отбор материала с целью подтверждения гипотезы; признать свою работу как неудачную и т. д. 

При длительных опытах рекомендуется периодически обсуждать их в научном коллективе. Это позволяет исследователю своевременно скорректировать ход эксперимента и направить его в нужном направлении [3]. 

Вопросы для самоконтроля 

1. Расскажите о теоретических исследованиях. 
2. В чем заключается различие между эмпирическим и теоретическим знанием? 
3. Модели теоретического исследования. 
4. Какова роль эксперимента в научном исследовании? 
5. Какие виды экспериментов вы знаете? 
6. В чем суть вычислительного эксперимента? 
7. Что в себя включает план эксперимента? 
8. Как планируется эксперимент? 
9. Что такое измерение? Его виды. 
10. Как организовать рабочее место экспериментатора?

Tất cả những điều trên chứng tỏ rằng bất kỳ kết quả thử nghiệm nào cũng phải được kiểm tra nhiều lần và nhìn nhận một cách nghiêm túc. Tốt hơn nên kiểm tra lại kết quả thử nghiệm vào một thời điểm khác trong ngày hoặc sau vài ngày.

Sau khi hoàn thành tất cả chuỗi thử nghiệm, nhà nghiên cứu có thể quyết định: có công nhận phần chính của tác phẩm là đã hoàn thành hay không; có cần tiến hành thu thập thêm thông tin và lựa chọn tài liệu để xác nhận giả thuyết không; thừa nhận công việc của bạn là không thành công, v.v.

Đối với các thí nghiệm dài hạn, nên thảo luận định kỳ trong nhóm khoa học. Điều này cho phép nhà nghiên cứu điều chỉnh kịp thời quá trình thí nghiệm và hướng nó đi đúng hướng [3].

Câu hỏi để tự kiểm soát

1. Hãy cho chúng tôi biết về nghiên cứu lý thuyết.
2. Sự khác biệt giữa kiến ​​thức thực nghiệm và lý thuyết là gì?
3. Mô hình nghiên cứu lý thuyết.
4. Vai trò của thí nghiệm trong nghiên cứu khoa học là gì?
5. Bạn biết những loại thí nghiệm nào?
6. Bản chất của thí nghiệm tính toán là gì?
7. Thiết kế thí nghiệm bao gồm những gì?
8. Thí nghiệm được lên kế hoạch như thế nào?
9. Đo lường là gì? Các loại của nó.
10. Tổ chức nơi làm việc của người thực nghiệm như thế nào?
